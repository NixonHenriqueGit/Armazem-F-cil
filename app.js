...
//  NAVEGACAO — PANES
// ══════════════════════════════════════════════════════
const ALL_PANES = ['menu','conf','emp','reports','despejo','repack','armazem','quebras','validades','dashboard','controle'];
...
let vldProdutos  = JSON.parse(localStorage.getItem('pk2_vld') || '[]');
let vldEditIndex = null;

function vldSave() {
  localStorage.setItem('pk2_vld', JSON.stringify(vldProdutos));
}

// ── Classificação de vencimento
function vldClassify(isoDate) {
  if (!isoDate) return { cls:'expired', label:'SEM DATA', days: null };
  const today = new Date(); today.setHours(0,0,0,0);
  const exp   = new Date(isoDate + 'T00:00:00');
  const days  = Math.ceil((exp - today) / 86400000);
  if (days <= 0)  return { cls:'expired', label:'VENCIDO',          days };
  if (days <= 30) return { cls:'crit',    label:'CRÍTICO',          days };
  if (days <= 45) return { cls:'warn',    label:'ATENÇÃO',          days };
  if (days <= 60) return { cls:'alert',   label:'ALERTA',           days };
  return            { cls:'ok',      label:'OK',               days };
}

function vldFmtDate(iso) {
  if (!iso) return '—';
...
function vldBuscarProduto(val) {
  const box = document.getElementById('vld-listaProdutos');
  if (!val || val.length < 2) { box.style.display='none'; return; }
  val = val.trim();
  let resultados;
  if (/^\d+$/.test(val)) {
    const exact = PRODUCTS.find(p => String(p.codigo) === val);
    if (exact) {
      document.getElementById('vld-codigo').value    = exact.codigo;
      document.getElementById('vld-descricao').value = exact.descricao;
      box.style.display='none'; return;
    }
    resultados = PRODUCTS.filter(p => String(p.codigo).startsWith(val));
  } else {
    const q = val.toLowerCase();
    resultados = PRODUCTS.filter(p => p.descricao.toLowerCase().includes(q));
  }
  ...
  box.innerHTML = resultados.map(p =>
    ...
    'onmousedown="vldSelecionarProduto(' + p.codigo + ')">' +
    '<strong>' + p.codigo + '</strong> — ' + p.descricao + '</div>'
  ).join('');
  box.style.display = 'block';
}

function vldSelecionarProduto(codigo) {
  const p = PRODUCTS.find(x => x.codigo === codigo);
  if (!p) return;
  document.getElementById('vld-produtoInput').value = p.codigo;
  document.getElementById('vld-codigo').value        = p.codigo;
  document.getElementById('vld-descricao').value     = p.descricao;
  document.getElementById('vld-listaProdutos').style.display = 'none';
}

// Fechar dropdown ao clicar fora
document.addEventListener('click', e => {
  const box = document.getElementById('vld-listaProdutos');
  if (box && !box.contains(e.target) && e.target.id !== 'vld-produtoInput') {
...
// ── Salvar / editar
function vldSalvar() {
  const codigo      = document.getElementById('vld-codigo').value.trim() ||
                      document.getElementById('vld-produtoInput').value.trim();
  const descricao   = document.getElementById('vld-descricao').value.trim();
  const palhete     = parseInt(document.getElementById('vld-palhete').value) || 0;
  const lastro      = parseInt(document.getElementById('vld-lastro').value)  || 0;
  const caixa       = parseInt(document.getElementById('vld-caixa').value)   || 0;
  const validade    = document.getElementById('vld-validade').value;
  const localizacao = document.getElementById('vld-localizacao').value;

  if (!codigo) { toast('Informe o código ou nome do produto', true); return; }
  if (!validade){ toast('Informe a data de validade', true); return; }

  const reg = { codigo, descricao, palhete, lastro, caixa, validade, localizacao, registradoEm: new Date().toISOString() };

  if (vldEditIndex !== null) {
    vldProdutos[vldEditIndex] = reg;
    vldEditIndex = null;
    document.getElementById('vld-btn-salvar').textContent = '💾 SALVAR PRODUTO';
  } else {
    vldProdutos.push(reg);
  }
  vldSave();
  vldLimparForm();
  vldRenderStats();
...
function vldLimparForm() {
  document.getElementById('vld-produtoInput').value = '';
  document.getElementById('vld-codigo').value       = '';
  document.getElementById('vld-descricao').value    = '';
  document.getElementById('vld-palhete').value      = '0';
  document.getElementById('vld-lastro').value       = '0';
  document.getElementById('vld-caixa').value        = '0';
  document.getElementById('vld-validade').value     = '';
  document.getElementById('vld-localizacao').value  = 'picking';
}

function vldCancelarEdicao() {
  vldEditIndex = null;
  document.getElementById('vld-btn-salvar').textContent = '💾 SALVAR PRODUTO';
  vldLimparForm();
}

function vldEditar(idx) {
  const p = vldProdutos[idx];
  if (!p) return;
  vldEditIndex = idx;
  document.getElementById('vld-produtoInput').value = p.codigo;
  document.getElementById('vld-codigo').value       = p.codigo;
  document.getElementById('vld-descricao').value    = p.descricao || '';
  document.getElementById('vld-palhete').value      = p.palhete || 0;
  document.getElementById('vld-lastro').value       = p.lastro  || 0;
  document.getElementById('vld-caixa').value        = p.caixa   || 0;
  document.getElementById('vld-validade').value     = p.validade;
  document.getElementById('vld-localizacao').value  = p.localizacao;
  document.getElementById('vld-btn-salvar').textContent = '✅ ATUALIZAR PRODUTO';
  // Ir para aba form
  ptabSwitch('validades','form');
  document.getElementById('vld-produtoInput').focus();
}

function vldExcluir(idx) {
  if (!confirm('Excluir este registro?')) return;
  vldProdutos.splice(idx, 1);
  vldSave();
  vldRender();
  vldRenderStats();
...
function vldLimparTudo() {
  if (!confirm('Apagar TODOS os registros de validade?')) return;
  vldProdutos = [];
  vldSave();
  vldRender();
  vldRenderStats();
...
// ── Stats dashboard
function vldRenderStats() {
  let exp=0,crit=0,warn=0,alert=0,ok=0;
  vldProdutos.forEach(p => {
    const {cls} = vldClassify(p.validade);
    if (cls==='expired') exp++;
    else if (cls==='crit') crit++;
    else if (cls==='warn') warn++;
    else if (cls==='alert') alert++;
    else ok++;
  });
  const s = id => document.getElementById(id);
  if (s('vs-exp')) { s('vs-exp').textContent=exp; s('vs-30').textContent=crit;
    s('vs-45').textContent=warn; s('vs-60').textContent=alert; s('vs-ok').textContent=ok; }
  const cnt = document.getElementById('vld-lista-cnt');
  if (cnt) cnt.textContent = vldProdutos.length;
}

// ── Render lista
function vldRender() {
  vldRenderStats();
  const body   = document.getElementById('vld-lista-body');
  if (!body) return;
  const fLoc    = document.getElementById('vld-f-loc')?.value    || 'todos';
  const fStatus = document.getElementById('vld-f-status')?.value || 'todos';
  const fSort   = document.getElementById('vld-f-sort')?.value   || 'asc';

  let lista = vldProdutos.map((p,i) => ({ ...p, _idx: i, ...vldClassify(p.validade) }));

  if (fLoc !== 'todos')    lista = lista.filter(p => p.localizacao === fLoc);
  if (fStatus !== 'todos') lista = lista.filter(p => p.cls === fStatus);

  if (fSort === 'asc')  lista.sort((a,b) => (a.validade||'').localeCompare(b.validade||''));
  else if (fSort==='desc') lista.sort((a,b) => (b.validade||'').localeCompare(a.validade||''));
  else lista.sort((a,b) => (a.localizacao||'').localeCompare(b.localizacao||'') || (a.validade||'').localeCompare(b.validade||''));

  if (!lista.length) {
    body.innerHTML = '<div class="vld-empty">📭 Nenhum produto encontrado com esses filtros</div>';
    return;
  }

  body.innerHTML = lista.map(p => {
    const daysTxt = p.days === null ? '—'
      : p.days <= 0 ? 'VENCIDO HÁ ' + Math.abs(p.days) + 'd'
      : 'VENCE EM ' + p.days + 'd';
    const locChip = '<span class="vld-loc-chip ' + p.localizacao + '">' + (p.localizacao||'').toUpperCase() + '</span>';
    return '<div class="vld-row ' + p.cls + '">' +
      '<div style="display:flex;flex-direction:column;gap:4px;flex:1;min-width:0">' +
        '<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">' +
          '<span class="vld-code">' + (p.codigo||'—') + '</span>' +
          '<span class="vld-badge ' + p.cls + '">' + p.label + '</span>' +
          locChip +
          '<span class="vld-days-pill" style="' + vldDayStyle(p.cls) + '">' + daysTxt + '</span>' +
        '</div>' +
        '<div class="vld-desc">' + (p.descricao || '—') + '</div>' +
        '<div class="vld-meta">' +
          '<span>📅 <strong>' + vldFmtDate(p.validade) + '</strong></span>' +
          (p.palhete ? '<span>🔲 <strong>' + p.palhete + '</strong> pal</span>' : '') +
          (p.lastro  ? '<span>📦 <strong>' + p.lastro  + '</strong> last</span>' : '') +
          (p.caixa   ? '<span>📫 <strong>' + p.caixa   + '</strong> cx</span>' : '') +
        '</div>' +
      '</div>' +
      '<div class="vld-actions">' +
        '<button class="btn btn-sm" style="background:rgba(168,85,247,.15);color:#a855f7" onclick="vldEditar(' + p._idx + ')">✏</button>' +
        '<button class="btn btn-sm btn-red" onclick="vldExcluir(' + p._idx + ')">🗑</button>' +
      '</div>' +
    '</div>';
  }).join('');
}

function vldDayStyle(cls) {
  if (cls==='expired') return 'background:rgba(127,29,29,.5);color:#fca5a5';
  if (cls==='crit')    return 'background:rgba(239,68,68,.15);color:var(--red)';
  if (cls==='warn')    return 'background:rgba(249,115,22,.15);color:#f97316';
  if (cls==='alert')   return 'background:rgba(234,179,8,.15);color:#eab308';
  return 'background:rgba(34,197,94,.1);color:var(--green)';
}

// ── Análise FEFO
function vldFefoRender() {
  const body = document.getElementById('vld-fefo-body');
  if (!body) return;
  if (!vldProdutos.length) {
    body.innerHTML = '<div class="vld-empty">📭 Sem registros para analisar</div>';
    return;
  }

  const picking = vldProdutos.filter(p => p.localizacao === 'picking');
  const central = vldProdutos.filter(p => p.localizacao === 'central');

  let html = '';

  // Tabela por localização
  ['picking','central'].forEach(loc => {
    const lista = (loc==='picking' ? picking : central)
      .map((p,_) => ({ ...p, _orig: vldProdutos.indexOf(p), ...vldClassify(p.validade) }))
      .sort((a,b) => (a.validade||'').localeCompare(b.validade||''));
    if (!lista.length) return;
    html += '<div class="card" style="margin-bottom:14px">';
    html += '<div class="card-title">' + (loc==='picking'?'🛒 PICKING':'🏭 CENTRAL') + ' — ' + lista.length + ' produto(s)</div>';
    html += '<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:12px">';
    html += '<thead><tr style="border-bottom:1px solid var(--border)">' +
      '<th style="padding:8px;text-align:left;color:var(--text-dim)">Código</th>' +
      '<th style="padding:8px;text-align:left;color:var(--text-dim)">Descrição</th>' +
      '<th style="padding:8px;text-align:center;color:var(--text-dim)">Validade</th>' +
      '<th style="padding:8px;text-align:center;color:var(--text-dim)">Pal/Last/Cx</th>' +
      '<th style="padding:8px;text-align:center;color:var(--text-dim)">Status</th>' +
      '</tr></thead><tbody>';
    lista.forEach(p => {
      html += '<tr style="border-bottom:1px solid var(--border2)">' +
        '<td style="padding:8px;font-family:\'Barlow Condensed\',sans-serif;font-weight:700">' + (p.codigo||'—') + '</td>' +
        '<td style="padding:8px;max-width:200px;word-break:break-word">' + (p.descricao||'—') + '</td>' +
        '<td style="padding:8px;text-align:center;font-weight:700">' + vldFmtDate(p.validade) + '</td>' +
        '<td style="padding:8px;text-align:center">' + (p.palhete||0) + '/' + (p.lastro||0) + '/' + (p.caixa||0) + '</td>' +
        '<td style="padding:8px;text-align:center"><span class="vld-badge ' + p.cls + '">' + p.label + '</span></td>' +
        '</tr>';
    });
    html += '</tbody></table></div></div>';
  });

  // Detectar quebras de FEFO (picking tem validade MAIOR que central para o mesmo produto)
  const quebras = [];
  picking.forEach(pk => {
    const lotesC = central.filter(c => String(c.codigo) === String(pk.codigo))
      .sort((a,b) => (a.validade||'').localeCompare(b.validade||''));
    if (lotesC.length && lotesC[0].validade < pk.validade) {
      quebras.push({ codigo: pk.codigo, descricao: pk.descricao,
        vPicking: pk.validade, vCentral: lotesC[0].validade });
    }
  });

  if (quebras.length) {
    html += '<div class="vld-fefo-alert"><strong>⚠ QUEBRA DE FEFO DETECTADA!</strong><br><br>';
    html += quebras.map(q =>
      '• Produto <strong>' + q.codigo + '</strong> (' + (q.descricao||'—') + '): ' +
      'picking vence em <strong>' + vldFmtDate(q.vPicking) + '</strong> mas central tem lote mais antigo: <strong>' + vldFmtDate(q.vCentral) + '</strong>. Priorize o lote do central.'
    ).join('<br>') + '</div>';
  } else if (vldProdutos.length) {
    html += '<div class="vld-fefo-ok">✅ <strong>Nenhuma quebra de FEFO detectada.</strong> Os lotes no picking são mais antigos que os do central para todos os produtos registrados.</div>';
  }

  body.innerHTML = html || '<div class="vld-empty">Sem dados suficientes para análise FEFO.</div>';
}

// ── Export Excel
function vldExportExcel() {
  if (!vldProdutos.length) { toast('Nenhum dado para exportar', true); return; }
  const dados = vldProdutos.map(p => {
    const {label, days} = vldClassify(p.validade);
    return {
      'Código':       p.codigo,
      ...
      'Lastros':      p.lastro  || 0,
      'Caixas':       p.caixa   || 0,
      'Validade':     vldFmtDate(p.validade),
      'Dias p/ vencer': days !== null ? days : 'N/A',
      'Status':       label,
  ...
  if (typeof XLSX !== 'undefined') {
    ...
    const rows   = dados.map(r => Object.values(r).join(';'));
    const csv    = [header, ...rows].join('\n');
    const blob   = new Blob(['\ufeff' + csv], {type:'text/csv;charset=utf-8'});
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = 'validades.csv';
    a.click();
